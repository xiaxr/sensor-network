...
private:
  ...
  bool _is_p_variant; /** For storing the result of testing the toggleFeatures()
                         affect */

  template <typename T, typename R>
  inline void transaction(const T &tbuf, R &rbuf) {
    beginTransaction();
    spi.transfernb(tbuf, rbuf);
    endTransaction();
  }
...
public:
   ...
   *
   * @note Users can specify default SPI speed by modifying `#define
   * RF24_SPI_SPEED` in RF24_config.h <br> For Arduino, SPI speed will only be
   * properly configured this way on devices supporting SPI TRANSACTIONS <br>
   * Older/Unsupported Arduino devices will use a default clock divider &
   * settings configuration <br> Linux: The old way of setting SPI speeds using
   * BCM2835 driver enums has been removed <br>
   *
   ...
   *   radio.available();                       // returned data should now be
   * reliable
   * }
   *
   * void setup() {
   *   pinMode(IRQ_PIN, INPUT);
   *   attachInterrupt(digitalPinToInterrupt(IRQ_PIN), isrCallbackFunction,
   * FALLING);
   * }
   * @endcode
   */
   ...
   * if radio.available():
   *     length = radio.getDynamicPayloadSize()  # or radio.getPayloadSize() for
   * static payload sizes received_payload = radio.read(length)
   * @endcode
   *
   ...
   * from pipe 1 per the assigned address width.<br> Pipes 1-5 should share the
   * same address, except the first byte. Only the first byte in the array
   ...
   * startListening(), but the address for pipe 0 is ONLY restored if the LSB is
   * a non-zero value.<br> Read
   ...
   * has_payload, pipe_number = radio.available_pipe()  # expand the tuple to 2
   * variables if has_payload: print("Received a payload with pipe",
   * pipe_number)
   * @endcode
   *
   ...
   *   radio.available(&pipe);                  // pipe data should now be
   * reliable
   * }
   *
   * void setup() {
   ...
   * sending. In full powerDown mode, the radio will consume approximately 900nA
   * (.0009mA)
   *
   * @code
   * radio.powerDown();
   ...
   *			radio.writeFast(&buf,32);  // Writes 1 payload to the
   *buffers
   *			txStandBy(1000);		   // Using extended
   *timeouts, returns 1 if success. Retries failed payloads for 1 seconds before
   *returning 0.
   * @endcode
   *
   * @see txStandBy()
   ...
   * @code
   * radio.writeBlocking(&buf, sizeof(buf), 1000); // Wait up to 1 second to
   * write 1 payload to the buffers radio.txStandBy(1000); // Wait up to 1
   * second for the payload to send. Return 1 if ok, 0 if failed.
   ...
   * by the manufacturer to drop the radio out of TX or STANDBY-II mode if there
   * is time enough between sends for the FIFOs to empty. This is not required
   ...
   * @param startTx If this is set to `true`, then this function puts the
   *nRF24L01 in TX Mode. `false` leaves the primary mode (TX or RX) as it is,
   *which can prevent the mandatory wait time to change modes.
   * @return
   * - `true` if all payloads in the TX FIFO were delivered successfully and
   *   an acknowledgement (ACK packet) was received for each. If auto-ack is
   *   disabled, then any attempt to transmit will also return true (even if
   ...
   * # let `radio` be the instantiated RF24 object
   * buffer = b"Hello World"  # a `bytes` object
   * radio.writeAckPayload(1, buffer)  # load an ACK payload for response on
   * pipe 1
   * @endcode
   *
   * @return
   ...
   * enabled, the nRF24L01 is never in TX mode long enough to disobey this rule.
   * Allow the FIFO to clear by issuing txStandBy() or ensure appropriate time
   ...
   */
  bool testCarrier(void) { return (read_register(CD) & 1); }
   ...
   * false if not.
   */
  bool testRPD(void) { return (read_register(RPD) & 1); }
   ...
   *
   * Writing to radio: Radio unresponsive - Fixed internally by adding a timeout
   *to the internal write functions in RF24 (failure handling)
   *
   ...
   *in  RF24Network.
   *
   * Radio configuration settings are lost - Fixed by monitoring a value that is
   *different from the default, and re-configuring the radio if this setting
   ...
   *addresses radio.openReadingPipe(1,addresses[0]); report_failure(); // Blink
   *leds, send a message, etc. to indicate failure
   *  }
   * @endcode
   */
   ...
   * Use 0 to disable the auto-retry feature all together.
   *
   ...
   * if its not.
   */
  bool isPVariant(void) { return _is_p_variant; }
   ...
   *
   * | @p level (enum value) | nRF24L01<br>description | Si24R1<br>description
   * when<br> @p lnaEnable = 1 | Si24R1<br>description when<br> @p lnaEnable = 0
   * |
   * |:---------------------:|:-------:|:--------:|:-------:|
   ...
   *
   * @note The getPALevel() function does not care what was passed @p lnaEnable
   * parameter.
   */
   ...
   *
   * Value resets with each new transmission. Allows roughly estimating signal
   * strength.
   *
   ...
   * @param length Specify one of the values (as defined by @ref
   * rf24_crclength_e) | @p length (enum value)     | description |
   * |:--------------------------:|:------------------------------:|
   ...
   *
   * The following code configures the IRQ pin to only reflect the "data
   * received" event:
   * @code
   * radio.maskIRQ(1, 1, 0);
   * @endcode
   ...
   * @param tx_fail  `true` ignores the "data failed" event, `false` reflects
   * the "data failed" event on the IRQ pin.
   * @param rx_ready `true` ignores the "data received" event, `false` reflects
   * the "data received" event on the IRQ pin.
   */
  ...
  /**
   * Transmission of constant carrier wave with defined frequency and output
   * power
   *
   ...
   * datasheet.
   * @note If isPVariant() returns true, please remember to re-configure the
   * radio's settings
   * @code
   ...
   * avoided as a reading pipe.<br> If used, the reading pipe 0 address needs to
   * be restored at avery call to startListening(), and the address<br> is ONLY
   * restored if the LSB is a non-zero value.<br> See
   * http://maniacalbits.blogspot.com/2013/04/rf24-addressing-nrf24l01-radios-require.html
   *
...
private:
   ...
   */
  void write_register(const uint8_t reg, const uint8_t *buf, uint8_t len);
