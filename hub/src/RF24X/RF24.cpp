...
RF24::RF24(uint16_t _cepin, uint16_t _cspin, uint32_t _spi_speed)
    : ce_pin(_cepin), csn_pin(_cspin), spi_speed(_spi_speed), payload_size(32),
      dynamic_payloads_enabled(true), addr_width(5), _is_p_variant(false),
      csDelay(5) {
  ...
  while ((get_status() &
    ...
    if (status & _BV(MAX_RT)) { // If MAX Retries have been reached
      ...
      if (millis() - timer > timeout) {
        return 0; // If this payload has exceeded the user-defined timeout, exit
                  // and return 0
      }
    }
