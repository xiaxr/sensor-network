...
RF24::RF24(uint16_t _cepin, uint16_t _cspin, uint32_t _spi_speed)
    : spi(_cspin, _spi_speed), ce_pin(_cepin), csn_pin(_cspin),
      spi_speed(_spi_speed), payload_size(32), dynamic_payloads_enabled(true),
      addr_width(5), _is_p_variant(false), csDelay(5) {
  pipe0_reading_address[0] = 0;
  if (spi_speed <=
      35000) { // Handle old BCM2835 speed constants, default to RF24_SPI_SPEED
...
uint8_t RF24::read_register(const uint8_t reg) {
  std::array<uint8_t, 2> ptx = {static_cast<uint8_t>(R_REGISTER | reg),
                                RF24_NOP};
  std::array<uint8_t, 2> prx = {0, 0};

  transaction(ptx, prx);

  status = prx[0]; // status is 1st byte of receive buffer
  return prx[1];   // result is 2nd byte of receive buffer
}

/****************************************************************************/

void RF24::write_register(const uint8_t reg, const uint8_t *buf, uint8_t len) {
  auto ptx = spi_buffer_t{};
  ptx.resize(len + 1);

  auto prx = spi_buffer_t{};
  prx.resize(len + 1);

  ptx[0] = (W_REGISTER | (REGISTER_MASK & reg));
  memcpy(ptx.data() + 1, buf, len);
...
void RF24::write_register(const uint8_t reg, const uint8_t value,
                          bool is_cmd_only) {
  if (is_cmd_only) {
    std::array<uint8_t, 1> ptx = {static_cast<uint8_t>(W_REGISTER | reg)};
    std::array<uint8_t, 1> prx = {0};
    transaction(ptx, prx);
    status = prx[0];
  } else {
    std::array<uint8_t, 2> ptx = {static_cast<uint8_t>(W_REGISTER | reg),
                                  value};
    std::array<uint8_t, 2> prx = {0, 0};
...
void RF24::write_payload(const void *buf, uint8_t data_len,
  ...
  auto size = data_len + blank_len + 1; // Add register value to transmit buffer
  auto ptx = spi_buffer_t{};
  ptx.resize(size);
  ...
  ptx[0] = writeType;
  memcpy(ptx.data() + 1, buf, data_len);
  std::fill_n(ptx.begin() + data_len, blank_len, 0);
...
void RF24::read_payload(void *buf, uint8_t data_len) {
  ...
  ptx[0] = R_RX_PAYLOAD;
  std::fill_n(ptx.begin() + 1, size - 1, RF24_NOP);
  ...
  if (data_len > 0) {
    memcpy(buf, prx.data() + 1, data_len);
  }
}
...
bool RF24::begin() {
  ...
  // datasheet for a more complete explanation.
  setRetries(5, 15);
  ...
  write_register(DYNPD,
                 0); // disable dynamic payloads by default (for all pipes)
...
//       "RF24 HARDWARE FAIL: Radio not responding, verify pin "
...
// Wait until complete or failed
  ...
  if (status & _BV(MAX_RT)) {
    flush_tx(); // Only going to be 1 packet in the FIFO at a time using this
                // method, so just flush
    return 0;
  }
  // TX OK 1 or 0
  return 1;
  ...
  while ((get_status() &
    ...
    if (status & _BV(MAX_RT)) { // If MAX Retries have been reached
      ...
      if (millis() - timer > timeout) {
        return 0; // If this payload has exceeded the user-defined timeout,
                  // exit and return 0
      }
    }
...
bool RF24::writeFast(const void *buf, uint8_t len, const bool multicast) {
  ...
  // Blocking only if FIFO is full. This will loop and block until TX is
  // successful or fail
  while ((get_status() & (_BV(TX_FULL)))) {
    if (status & _BV(MAX_RT)) {
      return 0; // Return 0. The previous payload has not been retransmitted
      // From the user perspective, if you get a 0, just keep trying to send the
      // same payload
    }
    if (millis() - timer > 95) {
      errNotify();
...
/****************************************************************************/
constexpr uint8_t child_pipe[] = {RX_ADDR_P0, RX_ADDR_P1, RX_ADDR_P2,
...
void RF24::setPALevel(uint8_t level, bool lnaEnable) {
  ...
  if (level > 3) { // If invalid level, go to max PA
    level = (RF24_PA_MAX << 1) +
            lnaEnable; // +1 to support the SI24R1 chip extra bit
  } else {
...
uint8_t RF24::getARC(void) { return read_register(OBSERVE_TX) & 0x0F; }
...
void RF24::startConstCarrier(rf24_pa_dbm_e level, uint8_t channel) {
  stopListening();
  write_register(RF_SETUP,
                 read_register(RF_SETUP) | _BV(CONT_WAVE) | _BV(PLL_LOCK));
  if (isPVariant()) {
    setAutoAck(0);
    setRetries(0, 0);
    ...
    write_register(W_TX_PAYLOAD, reinterpret_cast<const uint8_t *>(&dummy_buf),
                   32);
