
#include <cstddef>
#include <cstdint>
#include <mutex>

#include "bcm2835_defs.h"

#include "bcm2835.h"
#include "gpio.h"

#include "spi.h"

namespace {
pthread_mutex_t spiMutex = PTHREAD_MUTEX_INITIALIZER;
std::mutex spi_mutex;

constexpr uint8_t bcm2835_byte_reverse_table[] = {
    };
}  // namespace

namespace xiaxr {
namespace bcm2835 {
namespace impl {
uint8_t spi_impl::correct_order(const uint8_t b) {
  return (bit_order_ == BCM2835_SPI_BIT_ORDER_LSBFIRST)
             ? bcm2835_byte_reverse_table[b]
             : b;
}

/* Writes (and reads) a single byte to SPI */
uint8_t spi_impl::transfer(const uint8_t value) {
  volatile uint32_t *paddr =
      BCM2835::bcm2835()->bcm2835_spi0 + BCM2835_SPI0_CS / 4;
  volatile uint32_t *fifo =
      BCM2835::bcm2835()->bcm2835_spi0 + BCM2835_SPI0_FIFO / 4;
  uint32_t ret;

  /* This is Polled transfer as per section 10.6.1
  // BUG ALERT: what happens if we get interupted in this section, and someone
  else
  // accesses a different peripheral?
  // Clear TX and RX fifos
  */
  BCM2835::bcm2835()->peri_set_bits(paddr, BCM2835_SPI0_CS_CLEAR,
                                    BCM2835_SPI0_CS_CLEAR);

  /* Set TA = 1 */
  BCM2835::bcm2835()->peri_set_bits(paddr, BCM2835_SPI0_CS_TA,
                                    BCM2835_SPI0_CS_TA);

  /* Maybe wait for TXD */
  while (!(BCM2835::bcm2835()->peri_read(paddr) & BCM2835_SPI0_CS_TXD)) {
  };

  /* Write to FIFO, no barrier */
  BCM2835::bcm2835()->peri_write_nb(fifo, correct_order(value));

  /* Wait for DONE to be set */
  while (!(BCM2835::bcm2835()->peri_read_nb(paddr) & BCM2835_SPI0_CS_DONE)) {
  };

  /* Read any byte that was sent back by the slave while we sere sending to it
   */
  ret = correct_order(BCM2835::bcm2835()->peri_read_nb(fifo));

  /* Set TA = 0, and also set the barrier */
  BCM2835::bcm2835()->peri_set_bits(paddr, 0, BCM2835_SPI0_CS_TA);

  return ret;
}

/* Writes (and reads) an number of bytes to SPI */
void spi_impl::transfernb_(char *tbuf, char *rbuf, uint32_t len) {
  volatile uint32_t *paddr =
      BCM2835::bcm2835()->bcm2835_spi0 + BCM2835_SPI0_CS / 4;
  volatile uint32_t *fifo =
      BCM2835::bcm2835()->bcm2835_spi0 + BCM2835_SPI0_FIFO / 4;
  uint32_t TXCnt = 0;
  uint32_t RXCnt = 0;

  /* This is Polled transfer as per section 10.6.1
  // BUG ALERT: what happens if we get interupted in this section, and someone
  else
  // accesses a different peripheral?
  */

  /* Clear TX and RX fifos */
  BCM2835::bcm2835()->peri_set_bits(paddr, BCM2835_SPI0_CS_CLEAR,
                                    BCM2835_SPI0_CS_CLEAR);

  /* Set TA = 1 */
  BCM2835::bcm2835()->peri_set_bits(paddr, BCM2835_SPI0_CS_TA,
                                    BCM2835_SPI0_CS_TA);

  /* Use the FIFO's to reduce the interbyte times */
  while ((TXCnt < len) || (RXCnt < len)) {
    /* TX fifo not full, so add some more bytes */
    while (((BCM2835::bcm2835()->peri_read(paddr) & BCM2835_SPI0_CS_TXD)) &&
           (TXCnt < len)) {
      BCM2835::bcm2835()->peri_write_nb(fifo, correct_order(tbuf[TXCnt]));
      TXCnt++;
    }
    /* Rx fifo not empty, so get the next received bytes */
    while (((BCM2835::bcm2835()->peri_read(paddr) & BCM2835_SPI0_CS_RXD)) &&
           (RXCnt < len)) {
      rbuf[RXCnt] = correct_order(BCM2835::bcm2835()->peri_read_nb(fifo));
      RXCnt++;
    }
  }
  /* Wait for DONE to be set */
  while (!(BCM2835::bcm2835()->peri_read_nb(paddr) & BCM2835_SPI0_CS_DONE)) {
  };

  /* Set TA = 0, and also set the barrier */
  BCM2835::bcm2835()->peri_set_bits(paddr, 0, BCM2835_SPI0_CS_TA);
}
}  // namespace impl

void spi::beginTransaction() {
  pthread_mutex_lock(&spiMutex);
  impl_.set_data_mode(data_mode_);
  impl_.set_clock_divider(clock_speed_);
}

void spi::endTransaction() { pthread_mutex_unlock(&spiMutex); }
}  // namespace bcm2835
}  // namespace xiaxr